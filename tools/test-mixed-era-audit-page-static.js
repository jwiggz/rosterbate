...
expectMatch(/<section class="audit-grid">[\s\S]*<h2>Calibration Summary<\/h2>[\s\S]*<div id="calibrationSummary" class="metric-list"><\/div>/i, 'Calibration summary panel structure is missing');
...
expectMatch(/function getCalibrationBadgeClass\(label\)/i, 'getCalibrationBadgeClass helper is missing');
expectMatch(/function renderCalibrationSummary\(summary\)/i, 'renderCalibrationSummary hook is missing');
...
expectMatch(/var calibration = row && row\.calibration \? row\.calibration : null;/i, 'calibrated row cells are missing');
expectMatch(/var calibrationOvr = calibration \? calibration\['2kOverall'\] : null;/i, '2K OVR row cell is missing');
expectMatch(/var mismatchBadge = row && row\.calibrationMismatch/i, 'mismatch row cell is missing');
expectMatch(/getCalibrationBadgeClass\(row\.calibrationMismatch\)/i, 'calibration mismatch badge helper usage is missing');
